    ...
    <table>
        ...
        @foreach($cart as $cart)
            @foreach($cart->products as $product)
                <tr class="tr-row">
                    <td><img src="{{asset('products/'.$product->product_name)}}.jpg" width="90%" height="60%"
                             style="border: 1px solid silver;border-radius: 10px; max-height: 100px; max-width: 100px;">
                    </td>
                        ...
                        <div class="cart-button-01">
                            <div class="cart-button-02">
                                <form method="post" action="{{route('customer.quantityAdd',$product->id)}}">
                                    @csrf
                                    <button type="submit" class="operator-math">+</button>
                                </form>
                                ...
                                <form method="post" action="{{route('customer.quantitySub',$product->id)}}">
                                    @csrf
                                    <button type="submit" class="operator-math" style="margin-left: 4px">-</button>
                                </form>
                            </div>
                        </div>
                    </td>
